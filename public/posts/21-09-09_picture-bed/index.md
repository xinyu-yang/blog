...
因此就又在网上搜索解决之道，经琢磨了多篇解决方案之后，决定选择可能目前最合适的方案：使用PicGo作为图床管理软件，Gitee、SM.MS等作为存储平台，并且使用Onedrive来同步PicGo的配置文件和数据库（这是PicGo 2.3版本以来的新功能:clap:），以实现备份和同步。接下来将就该方案进行介绍：
## 申请Gitee账号，创建仓库
...
* repo (就是刚才所创建的仓库，不过这里的格式是“[your id]/[repo name]”，前面的gitee.com就不要填了)
* branch (一般为master)
...
可行的同步平台有很多，如 Onedrive、Dropbox、Google Drive、serfile (如果有的话)等。甚至Baidu云盘好像最近也免费提供了类似的服务“同步空间”，虽然每月只有1G的免费流量，但总比没有强。
